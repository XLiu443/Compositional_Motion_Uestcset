...
convert_label = {0:0, 1:1, 4:2, 5:3, 8:4, 11:5, 12:6, 16:7, 22:8, 28:9}
...
class Dataset(torch.utils.data.Dataset):
    ...
    def get_pose_data(self, data_index, frame_ix):
       ...
       # pose, ret_tr = self._load(data_index, frame_ix)
...
#        ipdb.set_trace()
        ...
        if index>=2470:
            inp, target = self._load_compose_pose(index)
            target = target - 30
            alpha = self.compose_alphas[index-2470]
            compose_video1 = self.compose_video1[index-2470]
            compose_video2 = self.compose_video2[index-2470]
        else:
            data_index = self.new_train_indices[index] 
            inp, target = self._get_item_data_index(data_index)
#            print("target", target)
            assert target in [0, 1, 4, 5, 8, 11, 12, 16, 22, 28]
            target = convert_label[target]
            alpha = -1
            compose_video1 = inp
            compose_video2 = inp 
       # inp, target = self._get_item_data_index(data_index)
        inp = inp.permute(1,2,0).contiguous()
        compose_video1 = compose_video1.permute(1,2,0).contiguous()
        compose_video2 = compose_video2.permute(1,2,0).contiguous()
        return inp, target, alpha, compose_video1, compose_video2
 
    def _load_compose_pose(self, data_index):
        ind = data_index-2470 
        inp = self.compose_poses[ind]
        target = self.compose_labels[ind] 
        return inp.float(), target
    ...
    def get_label_sample_batch(self, labels):
       # samples = [self.get_label_sample(label, n=1, return_labels=True, return_index=False) for label in labels]
        samples = [self.get_label_sample(label, n=5, return_labels=True, return_index=False) for label in labels]
      #  ipdb.set_trace()
    ...
    def __len__(self):
        ...
        if self.split == 'train':
            return min(len(self.new_train), num_seq_max)
        else:
    ...
    def update_parameters(self, parameters):
     ...
     #   _, self.njoints, self.nfeats = self[0][0].shape
